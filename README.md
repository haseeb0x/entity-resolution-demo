...
The problem is that these systems were not designed with Arabic names in mind.

High-frequency given names (Muhammad, Ahmad), patronymic structures (bin/ibn), tribal markers (al-Rashidi), nasab chains, and transliteration variability (عبدالله → Abdullah / Abdallah / Abd Allah) produce structurally higher collision rates in any string-matching system. This is compounded by the fact that since 9/11 and the Patriot Act, a disproportionate share of entries on the OFAC SDN list are Arabic names — many improperly transliterated, carrying high numbers of variations and alternative monikers. Financial institutions face severe penalties for missed matches, which cultivates a culture of over-flagging. The result: astronomical false positive rates, cascading into screening failures and eventual debanking. The discrimination becomes structural, not intentional.
